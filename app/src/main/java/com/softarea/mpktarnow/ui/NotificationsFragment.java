...
public class NotificationsFragment extends Fragment {
  ...
  public View onCreateView(@NonNull LayoutInflater inflater,
    ...
    if (MainActivity.lat_from == null && MainActivity.lat_to == null) {
      GeoUtils.getCurrentLocation(handlerUpdateFirstField, getContext());
    }
    ...
    buttonSearchOnMapTo.setOnClickListener(view -> {
      ...
      Navigation.findNavController(root).navigate(R.id.navigation_get_coords_map, result);
    });
    ...
    btnSearchbyLocationTo.setOnClickListener(view -> {
      GeoUtils.getCurrentLocation(handlerUpdateSecondField, getContext());
    });
  ...
  private void setPositionFrom() {
    if (MainActivity.lat_from != null && MainActivity.lng_from != null) {
      try {
        startPoint.setText(GeoUtils.getPlaceFromCoords(getContext(), new LatLng(MainActivity.lat_from, MainActivity.lng_from)));
      } catch (IOException e) {
        e.printStackTrace();
      }
    }
  ...
  private void setPositionTo() {
    if (MainActivity.lat_to != null && MainActivity.lng_to != null) {
      try {
        endPoint.setText(GeoUtils.getPlaceFromCoords(getContext(), new LatLng(MainActivity.lat_to, MainActivity.lng_to)));
      } catch (IOException e) {
        e.printStackTrace();
      }
    }
  ...
  private final Handler handlerUpdateFirstField = new Handler() {
    @Override
  ...
  //UPDATE_SECOND
  @SuppressLint("HandlerLeak")
  private final Handler handlerUpdateSecondField = new Handler() {
    @Override
    public void handleMessage(Message msg) {
      LatLng pos = (LatLng) msg.obj;
      MainActivity.lat_current = pos.latitude;
